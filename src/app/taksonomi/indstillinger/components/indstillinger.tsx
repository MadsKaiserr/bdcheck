...
export default function Indstillinger({ kategorier }: any) {
    ...
    const [originalMatchObject, setOriginalMatchObject] = useState(kategorier);
    const [matchObject, setMatchObject] = useState(kategorier)
    ...
    const toggleMatch = (slug: string) => {
        setOpenMatches((prev) => ({ ...prev, [slug]: !prev[slug] }));
    };
    ...
    async function saveChanges() {
        setLoading(true)
        try {
            const res = await fetch("/api/gem-taksonomi", {
                method: "POST",
