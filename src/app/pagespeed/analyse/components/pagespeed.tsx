...
export default function Pagespeed({ kategorier, domain, pagespeedData }: PagespeedProps) {
    ...
    const headerText = `The mobile performance score is ${pagespeedData.performanceScore}. This is a result below our initial target of >70. We therefore see the following optimization potential for the website:`;
    ...
    useEffect(() => {
        if (pagespeedData && kategorier.length > 0) {
            const issuesToCheck: string[] = [];

            for (const kategori of kategorier) {
                for (const issue of kategori.issues) {
                    if (pagespeedData.audits[issue.psi_id] && issue.trigger !== undefined) {
                        if (pagespeedData.audits[issue.psi_id].score < issue.trigger) {
                            issuesToCheck.push(issue.id);
                        }
                    }

                    if (pagespeedData.audits["lcp-discovery-insight"]) {
                        if (issue.psi_id == "lcp-discovery-fetchpriority") {
                            if (!pagespeedData.audits["lcp-discovery-insight"].details.items[0].items.priorityHinted.value && pagespeedData.audits["lcp-discovery-insight"].score < issue.trigger) {
                                issuesToCheck.push(issue.id);
                            }
                        }
                        if (issue.psi_id == "lcp-discovery-eager") {
                            if (!pagespeedData.audits["lcp-discovery-insight"].details.items[0].items.eagerlyLoaded.value && pagespeedData.audits["lcp-discovery-insight"].score < issue.trigger) {
                                issuesToCheck.push(issue.id);
                            }
                        }
                    }

                    if (pagespeedData.audits["image-delivery-insight"]) {
                        const image_items = pagespeedData.audits["image-delivery-insight"]?.details?.items || [];
                        if (issue.psi_id == "lcp-images-oldformats" && pagespeedData.audits["image-delivery-insight"].score < issue.trigger) {
                        ...
                        if (issue.psi_id == "lcp-images-toolarge" && pagespeedData.audits["image-delivery-insight"].score < issue.trigger) {
                        ...
                        if (issue.psi_id == "lcp-images-intrinsicsize" && pagespeedData.audits["image-delivery-insight"].score < issue.trigger) {
                            const intrinsicSizeCount = image_items.filter((item: { node: { boundingRect: { width: any; }; }; url: string | URL; }) => {
                                ...
                                // Vi trækker bredden ud af URL'en (f.eks. ?width=860)
                                const urlParams = new URL(item.url).searchParams;
                                const intrinsicWidth = parseInt(urlParams.get("width") || "0");

                                // Tjek om den originale bredde er >= dobbelt af den viste bredde
    ...
    }, [pagespeedData, kategorier]);

    const outputText = kategorier
    .flatMap((kategori: any) =>
      kategori.issues.filter((issue: any) => checkedIssues.includes(issue.id))
    )
                ...
                <div className="pagespeed__container">
                    <div className="pagespeed__status__container">
                        ...
                        className={`pagespeed__status__performancescore__container ${
                            pagespeedData.performanceScore >= 0 && pagespeedData.performanceScore <= 49
                            ? "pagespeed__status__performancescore__poor"
                            : pagespeedData.performanceScore >= 50 && pagespeedData.performanceScore <= 89
                            ? "pagespeed__status__performancescore__fair"
                            : pagespeedData.performanceScore >= 90 && pagespeedData.performanceScore <= 100
                    ...
                    {pagespeedData && 
                        <div className="pagespeed__diagnostics__wrapper">
                            {(() => {
                                if (!pagespeedData || !pagespeedData.audits) return null;
                                ...
                                // 2. Loop alle audits
                                Object.entries(pagespeedData.audits).forEach(([id, audit]: [string, any]) => {
                                if (audit.score === null || audit.score >= 0.9) return; // skjul gode audits

                                // find auditRef for group og weight
                                const ref = pagespeedData.auditRefs?.find((r: any) => r.id === id);
