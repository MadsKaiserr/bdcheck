...
/**
 * Hjælpefunktion til at dekode HTML entities som &quot;, &amp; osv.
 * Dette er kritisk når CMS'er encoder indholdet i script-tags.
 */
...
/**
 * Rekursiv funktion der graver alle @type værdier ud af et objekt,
 * uanset om det er gemt i @graph, arrays eller nested properties.
 */
function extractTypes(obj: any, typesList: string[]) {
    if (!obj || typeof obj !== 'object') return;

    // Hvis det er et array, kør funktionen på hvert element
    if (Array.isArray(obj)) {
        obj.forEach((item) => extractTypes(item, typesList));
        return;
    }

    // Find @type (kan være en streng eller et array)
    if (obj["@type"]) {
    ...
    // Tjek @graph (specielt for Yoast/WordPress)
    if (obj["@graph"]) {
        extractTypes(obj["@graph"], typesList);
    }

    // Gennemgå alle andre nøgler for at finde nested objekter (f.eks. aggregateRating)
...
export async function POST(req: Request) {
    try {
        ...
        const response = await fetch(url, {
            ...
            next: { revalidate: 0 }, // Undgå Next.js cache
        });
        ...
        $('script[type="application/ld+json"]').each((_, element) => {
            ...
            // 1. Dekod HTML-tegn (Fixer &quot; problemet)
            rawText = decodeHtmlEntities(rawText);

            // 2. Rens for JS/HTML-støj der ødelægger JSON.parse
            ...
            try {
                // Find det faktiske startpunkt for JSON ({ eller [) 
                // i tilfælde af at der er tekst før objektet.
